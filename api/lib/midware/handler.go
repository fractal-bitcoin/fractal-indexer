...
func Metrics() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		ServiceMetrics.Inc("flight", "in") // Track the running API request.
		...
		if isMetricsOn {
			...
			ServiceMetrics.Observe(strconv.Itoa(statusCode), "httpcode", latency) // Count incoming requests by HTTP code.
			...
			if statusCode != 404 {
				...
				ServiceMetrics.Observe(urlPattern, "url", latency) // Count incoming requests by HTTP URL.
