...
func NewBRC20ModuleHistory(isTransfer bool, historyType uint8, from, to *InscriptionBRC20Data, data any, isValid bool) *BRC20ModuleHistory {
	...
	if isTransfer {
		history.OldSatpoint = fmt.Sprintf("%s:%d:%d", utils.HashString([]byte(from.TxId)), from.Vout, from.Offset)
		history.NewSatpoint = fmt.Sprintf("%s:%d:%d", utils.HashString([]byte(to.TxId)), to.Vout, to.Offset)
		...
		history.OldSatpoint = ""
		history.NewSatpoint = fmt.Sprintf("%s:%d:%d", utils.HashString([]byte(from.TxId)), from.Vout, from.Offset)
...
// commit history
type BRC20SwapHistoryCommitData struct {
	FunctionsTotal int   `json:"total"`
	InvalidList    []int `json:"invalid"`
}
