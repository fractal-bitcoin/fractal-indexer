...
func TestNewDecimalFromString(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.input, func(t *testing.T) {
			got, err := decimal.FromString(tc.input, 18)
			if (err != nil) != tc.err {
...
func TestDecimal_Add(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.a+"+"+tc.b, func(t *testing.T) {
			da, _ := decimal.FromString(tc.a, 18)
			db, _ := decimal.FromString(tc.b, 18)
			got := da.Add(db)
...
func TestDecimal_String(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.input, func(t *testing.T) {
			d, _ := decimal.FromString(tc.input, 18)
			got := d.String()
			if got != tc.want {
...
func BenchmarkAdd(b *testing.B) {
	d1, _ := decimal.FromString("123456789.123456789", 18)
	d2, _ := decimal.FromString("987654321.987654321", 18)
	for n := 0; n < b.N; n++ {
		d1.Add(d2)
	}
}
