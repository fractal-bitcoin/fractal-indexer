...
func NewDecimal(v uint64, p uint8) *Decimal {
	if p > MAX_PRECISION {
		p = MAX_PRECISION
	}
	return &Decimal{Precision: p, Value: new(big.Int).SetUint64(v)}
}

func FromUint128(u uint128.Decimal) *Decimal {
	if u.IsZero() {
		return nil
	}
	hi := NewDecimal(u.Hi, u.Precision)
	lo := NewDecimal(u.Lo, u.Precision)
	m64 := NewDecimal(math.MaxUint64, u.Precision)
	return hi.Mul(m64).Add(lo)
}

// FromString creates a Decimal instance from a string
func FromString(s string, maxPrecision int) (*Decimal, error) {
	if s == "" {
	...
	return &Decimal{Precision: uint8(maxPrecision), Value: value}, nil
}
...
func (d *Decimal) Sub(other *Decimal) *Decimal {
	...
	if d.Precision != other.Precision {
		panic(fmt.Sprintf("precision not match, (%d != %d)", d.Precision, other.Precision))
	}
...
func (d *Decimal) Mul(other *Decimal) *Decimal {
	...
	// value := new(big.Int).Div(value0, precisionFactor[other.Precision])
	return &Decimal{Precision: d.Precision, Value: value}
}
...
func (d *Decimal) Sqrt() *Decimal {
	...
	// value0 := new(big.Int).Mul(d.Value, precisionFactor[d.Precision])
	value := new(big.Int).Sqrt(d.Value)
	return &Decimal{Precision: MAX_PRECISION, Value: value}
}
...
func (d *Decimal) Div(other *Decimal) *Decimal {
	...
	// value0 := new(big.Int).Mul(d.Value, precisionFactor[other.Precision])
...
func (d *Decimal) Uint128() (u uint128.Decimal) {
	if d == nil {
		return u
	}

	return uint128.FromBig(d.Value, d.Precision)
}
