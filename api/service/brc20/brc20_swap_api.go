...
func GetBRC20ModuleHistory(historyType, module string, startHeight, endHeight, start, size int) (total int, nftsRsp []*model.BRC20ModuleHistoryInfo, err error) {
	...
	var moduleInfoHistory []*swapModel.BRC20ModuleHistory = moduleInfo.History // fixme: support type filter
...
// for api, history
func GetBRC20ModuleWithdrawHistory(startHeight, endHeight, start, size int) (total int, nftsRsp []*model.BRC20ModuleHistoryInfo, err error) {
	logger.Log.Info("GetBRC20ModuleWithdrawHistory",
		zap.Int("startHeight", startHeight),
		zap.Int("endHeight", endHeight),
		zap.Int("start", start),
		zap.Int("size", size))
	...
	var moduleInfoHistory []*swapModel.BRC20ModuleHistory = model.GSwap.AllModulesWithdrawHistory
	...
	for idx := start; idx < total; idx++ {
		...
		historyResp := &model.BRC20ModuleHistoryInfo{
			...
			OldSatpoint: history.OldSatpoint,
			NewSatpoint: history.NewSatpoint,
