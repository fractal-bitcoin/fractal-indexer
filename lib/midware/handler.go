...
func Metrics() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if strings.HasPrefix(path, "/health/") || strings.HasPrefix(path, "/metrics") {
			// Do not record metrics for health and metrics endpoints.
		...
		if isMetricsOn {
			...
			ServiceMetrics.Observe(strconv.Itoa(statusCode), "httpcode", latency) // Count incoming requests by HTTP status code.
			ServiceMetrics.Observe(strconv.Itoa(bizCode), "bizcode", latency)     // Count incoming requests by business code.
			if statusCode != 404 {
				urlPattern := c.FullPath()
				ServiceMetrics.Observe(urlPattern, "url", latency) // Count incoming requests by HTTP URL pattern.
