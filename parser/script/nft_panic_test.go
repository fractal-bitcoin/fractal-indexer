...
func firstExtractPkScriptForNFT(pkScript []byte) (nft *NFTData, hasNFT bool) {
	nfts := ExtractPkScriptForNFTJubilee(pkScript, nil)
	if len(nfts) == 0 {
		return nil, false
	}
	return &nfts[0], true
...
func TestMinimalEnvelope(t *testing.T) {
	...
	_, hasNFT := ExtractPkScriptForNFT(script)
...
func TestPUSHDATA2EmptyStringTagPanic(t *testing.T) {
	var script []byte
...
func TestJubileeStutteringDetection(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			nfts := ExtractPkScriptForNFTJubilee(tt.script, nil)
			if len(nfts) != 1 {
