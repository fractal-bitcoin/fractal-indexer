...
// pre-allocated single-byte slices to avoid heap allocation in GetOpcodeFormScript
var singleBytes [256][]byte
var emptyBytes = make([]byte, 0, 0)

func init() {
	for i := range singleBytes {
		singleBytes[i] = []byte{byte(i)}
	}
}
...
func GetOpcodeFormScript(raw []byte) (size uint, data []byte, isPush, isOpcode bool) {
	...
	c := raw[0]
	if c > txscript.OP_16 {
		return 1, singleBytes[c], false, true
	}

	if c >= txscript.OP_1 {
		return 1, singleBytes[c-txscript.OP_1+1], true, true
	}

	if c == txscript.OP_0 {
		return 1, emptyBytes, true, true
	}
