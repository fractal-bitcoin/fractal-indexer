...
func ExtractPkScriptForNFTJubilee(pkScript []byte, nfts []NFTData) []NFTData {
	length := len(pkScript)
	if length == 0 {
		return nfts
	}
	...
	for p < e {
		...
		nft := NFTData{IsStutter: stutter} // value type, no heap alloc
		...
		for offset < e {
			...
			if len(dataTag) == 0 { // ORDINALS_TAG_CONTENT
				...
				// Always restart after processing the content.
				break
			}
			...
			if data == nil {
				...
				if !isDupTag {
					if dataTag[0] == ORDINALS_TAG_CONTENTTYPE {
						...
						if _, ok := GetNFTIdFromScript(data); ok {
							nft.ParentsId = append(nft.ParentsId, string(data))
